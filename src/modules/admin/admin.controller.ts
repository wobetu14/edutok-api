...
export async function listPendingCourses(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const data = await service.listPendingCourses(page, limit);
    ok(res, data);
  } catch (e) { next(e); }
}
...
export async function reviewCourse(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const data = await service.reviewCourse(req.params.id, req.user!.id, status, notes);
    ok(res, data);
  } catch (e) { next(e); }
}
...
export async function createAnnouncement(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const data = await service.createAnnouncement(req.user!.id, title, body, expires_at);
    created(res, data);
  } catch (e) { next(e); }
}
